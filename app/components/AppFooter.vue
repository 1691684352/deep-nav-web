...
<template>
  <footer v-if="site.site" class="mt-6 border-t bg-muted/30">
    <div class="page-shell grid gap-10 py-10 lg:grid-cols-[1.4fr_2fr_1fr]">
      <div>
        <NuxtLink to="/" class="inline-flex items-center gap-2.5">
          <AppLogo :size="26" />
          <span class="font-display text-[17px] font-extrabold tracking-tight">{{ site.site.name }}</span>
        </NuxtLink>
        ...
        <div v-if="site.site.footerSocials.length" class="mt-4 flex gap-2">
          <template v-for="social in site.site.footerSocials" :key="social.id">
            <a
              ...
              class="grid size-9 place-items-center rounded-full border text-muted-foreground transition-colors hover:border-foreground/30 hover:text-foreground"
              :href="social.to"
      ...
      <div class="grid grid-cols-3 gap-6 text-[13px] leading-6 text-muted-foreground">
        <div v-for="column in site.site.footerColumns" :key="column.title">
          <p class="mb-3 text-[14px] font-bold text-foreground">{{ column.title }}</p>
      ...
      <div v-if="site.site.footerFollow.qrCodes.length">
        ...
        <div class="mt-3 flex gap-4">
          <div
            ...
            :key="qr.id"
            class="grid size-[78px] place-items-center overflow-hidden rounded-lg border bg-background"
          >
            <img
