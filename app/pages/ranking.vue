    ...
    <section id="ranking" class="ranking-center min-w-0 space-y-4" aria-labelledby="ranking-title">
      <section class="ranking-hero panel rounded-xl p-7 sm:p-8">
        <div class="ranking-hero__copy max-w-[620px]">
          ...
          <h1 id="ranking-title" class="mt-2 font-display text-[30px] font-extrabold leading-tight text-ink">排行榜</h1>
          <p class="mt-2 max-w-[520px] text-[13px] font-medium leading-6 text-copy">
          ...
          <div class="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-[11px] text-muted">
            <span v-for="stat in data?.stats ?? []" :key="stat.label">
              <strong class="number-font mr-1 text-[13px] text-ink">{{ stat.value }}</strong>{{ stat.label }}
            </span>
          </div>
        </div>
      ...
      <section class="panel rounded-xl p-5" aria-labelledby="ranking-list-title">
        <div class="flex items-start justify-between gap-4">
          <div>
            <h2 id="ranking-list-title" class="font-display text-[18px] font-bold text-ink">全站热度排行</h2>
            <p class="mt-1 text-[12px] text-muted">{{ ranking?.summary }}</p>
          </div>
          <button
            class="grid size-9 shrink-0 place-items-center rounded-lg text-muted transition hover:bg-canvas hover:text-brand"
            type="button"
            aria-label="刷新排行榜"
        ...
        <div v-if="podium.length" class="ranking-podium mt-5" aria-label="排行榜前三名">
          ...
          >
            ...
            <span class="podium-card__meta">
              <AppIcon name="flame" class="mr-0.5 inline size-3 text-[#ef5b59]" />{{ entry.heat }}
            </span>
          </NuxtLink>
        </div>
      ...
      <section class="panel rounded-xl p-5" aria-labelledby="ranking-recommend-title">
        <div class="flex items-center justify-between gap-4">
          <div>
            <h2 id="ranking-recommend-title" class="font-display text-[17px] font-bold text-ink">编辑推荐</h2>
            <p class="mt-1 text-[12px] text-muted">兼顾实用性与产品体验的优质工具</p>
          </div>
          <NuxtLink to="/category/all" class="flex shrink-0 items-center gap-1 text-[12px] font-semibold text-brand">
    ...
    <aside class="ranking-right-rail right-rail space-y-4" aria-label="排行榜说明与分类榜单">
      <section class="panel rounded-xl p-5">
        <div class="flex items-center gap-2">
          <AppIcon name="badge-info" class="size-4 text-brand" />
          <h2 class="font-display text-[16px] font-bold text-ink">榜单说明</h2>
        </div>
        ...
        <NuxtLink to="/about/help" class="mt-4 flex items-center gap-1 text-[12px] font-semibold text-brand">
      ...
      <section class="panel rounded-xl p-5" aria-labelledby="category-ranking-title">
        <div class="flex items-center justify-between gap-3">
          <h2 id="category-ranking-title" class="font-display text-[16px] font-bold text-ink">分类榜单</h2>
          <NuxtLink to="/category/all" class="text-[11px] font-semibold text-brand">查看全部</NuxtLink>
        </div>
        <div class="mt-3 space-y-1">
          ...
          >
            <span class="number-font w-4 text-center text-[11px] font-bold text-[#8792a8]">{{ index + 1 }}</span>
            <span class="relative grid size-[26px] shrink-0 place-items-center overflow-hidden rounded-lg bg-brand-soft text-[10px] font-bold text-brand">
            ...
            <span class="min-w-0 flex-1">
              <strong class="block truncate text-[12px] font-medium text-ink">{{ item.category }}榜</strong>
              <span class="mt-0.5 block truncate text-[10px] leading-4 text-muted">{{ item.topTool.name }} · {{ item.topTool.heat }}</span>
            </span>
            <span class="number-font flex items-center justify-end gap-0.5 text-[11px] font-semibold text-[#ff534c]">
      ...
      <section class="panel rounded-xl p-5" aria-labelledby="rising-ranking-title">
        <div class="flex items-center justify-between gap-3">
          <h2 id="rising-ranking-title" class="font-display text-[16px] font-bold text-ink">上升最快</h2>
          <span class="rounded-md bg-[#fff0f2] px-1.5 py-1 text-[10px] font-bold text-[#ef5b67]">实时</span>
        </div>
        <div class="mt-3">
          <NuxtLink
          ...
          >
            <span
              ...
              :class="item.rank <= 3 ? 'text-[#ff5145]' : 'text-[#8792a8]'"
            >{{ item.rank }}</span>
            <span class="relative grid size-[26px] shrink-0 place-items-center overflow-hidden rounded-lg bg-brand-soft text-[10px] font-bold text-brand">
            ...
            <span class="min-w-0 flex-1">
              <strong class="block truncate text-[12px] font-medium text-ink">{{ item.tool.name }}</strong>
