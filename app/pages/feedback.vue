...
<template>
  <div class="grid gap-5 lg:grid-cols-[220px_minmax(0,1fr)_260px]">
    ...
    <section class="panel rounded-xl p-6" aria-labelledby="feedback-page-title">
      <p class="text-[11px] font-bold uppercase tracking-[0.18em] text-brand">Feedback</p>
      <h1 id="feedback-page-title" class="mt-2 font-display text-[24px] font-extrabold text-ink">建议反馈</h1>
      <p class="mt-2 text-[13px] leading-6 text-copy">你的建议会直接进入产品反馈队列，帮助我们持续改善导航内容和使用体验。</p>

      <form class="mt-6 space-y-5" @submit.prevent="submit">
        <div>
          <label for="feedbackType" class="text-[12px] font-semibold text-ink">反馈类型</label>
        ...
        <div>
          <label for="feedbackContent" class="text-[12px] font-semibold text-ink">反馈内容</label>
          <textarea
            id="feedbackContent"
        ...
        <div>
          <label for="feedbackContact" class="text-[12px] font-semibold text-ink">联系方式</label>
          <input
            id="feedbackContact"
        ...
        <p v-if="error" class="text-[12px] font-medium text-[#e25760]">{{ error }}</p>
    ...
    <aside class="panel h-fit rounded-xl p-5">
      <h2 class="font-display text-[15px] font-bold text-ink">反馈说明</h2>
      <ul class="mt-4 space-y-3 text-[12px] leading-6 text-copy">
        <li v-for="notice in data?.notices ?? []" :key="notice" class="flex gap-2">
          <AppIcon name="circle-check" class="mt-1 size-3.5 shrink-0 text-brand" />
          <span>{{ notice }}</span>
        </li>
      </ul>
    </aside>
