  ...
  <ProfileShell :nav-items="data?.navItems ?? []">
    ...
    <BaseDialog :open="Boolean(active)" dialog-class="global-search-dialog" aria-labelledby="submissionDetailTitle" @close="active = null">
      <div class="global-search-panel">
        ...
        <dl v-if="active" class="grid gap-3 p-5 text-[13px] text-copy sm:grid-cols-2">
          <div><dt class="text-[11px] text-muted">网站名称</dt><dd class="mt-1 font-semibold text-ink">{{ active.name }}</dd></div>
          <div><dt class="text-[11px] text-muted">网站地址</dt><dd class="mt-1 truncate font-semibold text-ink">{{ active.url }}</dd></div>
          <div><dt class="text-[11px] text-muted">分类</dt><dd class="mt-1 font-semibold text-ink">{{ active.category }}</dd></div>
          ...
          <div><dt class="text-[11px] text-muted">提交时间</dt><dd class="mt-1 font-semibold text-ink">{{ active.submittedAt }}</dd></div>
          <div><dt class="text-[11px] text-muted">更新时间</dt><dd class="mt-1 font-semibold text-ink">{{ active.updatedAt }}</dd></div>
          <div class="sm:col-span-2"><dt class="text-[11px] text-muted">一句话描述</dt><dd class="mt-1 text-ink">{{ active.slogan }}</dd></div>
